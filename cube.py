...
class Cube:
    ...
    default_face_colors = ["white", "yellow",
                           "blue", "green",
                           "orange", "red",
                           "gray", "none"]
    ...
    def __init__(self, plastic_color=None, face_colors=None):
        self.N = 3
...
class InteractiveCube(plt.Axes):
    def __init__(self, cube=None,
        ...
        self.figure.text(0.05, 0.18,
                         "Mouse to move a Cube\n"
                         "U/D/L/R/B/F keys turn Cube\n",
                         size=10)

    def _initialize_widgets(self):
        self._ax_solve = self.figure.add_axes([0.1, 0.05, 0.25, 0.075])
        self._btn_solve = widgets.Button(self._ax_solve, 'Solve Cube')
        self._btn_solve.on_clicked(self._solve_cube)

        self._ax_god_algorithm = self.figure.add_axes([0.4, 0.05, 0.25, 0.075])
        self._btn_god_algorithm = widgets.Button(self._ax_god_algorithm, 'God\'s Algorithm')
        self._btn_god_algorithm.on_clicked(self._god_algorithm)

        self._ax_reset = self.figure.add_axes([0.7, 0.05, 0.25, 0.075])
